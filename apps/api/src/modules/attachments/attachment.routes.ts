...
attachmentRoutes.post("/attachments/:id/extraction", authenticate, requirePermission("attachments.create"), authenticatedAsyncHandler((request, response) => attachmentController.requestExtraction(request, response)));
attachmentRoutes.get("/attachments/:id/extraction", authenticate, requirePermission("attachments.read"), authenticatedAsyncHandler((request, response) => attachmentController.getExtractionStatus(request, response)));
attachmentRoutes.post("/attachments/:id/extraction/retry", authenticate, requirePermission("attachments.create"), authenticatedAsyncHandler((request, response) => attachmentController.retryExtraction(request, response)));
attachmentRoutes.get("/attachments/:id/extraction/text", authenticate, requirePermission("attachments.read"), authenticatedAsyncHandler((request, response) => attachmentController.getExtractedText(request, response)));
