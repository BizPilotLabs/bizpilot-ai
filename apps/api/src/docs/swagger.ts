...
const schemas = {
  UserSummary: entity({ id: uuid, email: { type: "string", format: "email" }, firstName: { type: "string" }, lastName: { type: "string" }, avatar: nullableString }),
  ...
  Project: entity({ id: uuid, organizationId: uuid, name: { type: "string" }, description: nullableString, status: { type: "string", enum: ["PLANNED", "ACTIVE", "ON_HOLD", "COMPLETED", "CANCELLED"] }, startDate: { type: ["string", "null"], format: "date-time" }, endDate: { type: ["string", "null"], format: "date-time" }, color: nullableString, archived: { type: "boolean" }, createdById: uuid, createdBy: { $ref: "#/components/schemas/UserSummary" }, taskCount: { type: "integer" }, completedTaskCount: { type: "integer" }, progressPercentage: { type: "integer", minimum: 0, maximum: 100 }, createdAt: dateTime, updatedAt: dateTime }),
  Task: entity({ id: uuid, projectId: uuid, project: { type: "object", properties: { id: uuid, name: { type: "string" }, status: { type: "string" }, archived: { type: "boolean" } }, required: ["id", "name", "status", "archived"] }, title: { type: "string" }, description: nullableString, status: { type: "string", enum: ["TODO", "IN_PROGRESS", "IN_REVIEW", "DONE", "CANCELLED"] }, priority: { type: "string", enum: ["LOW", "MEDIUM", "HIGH", "CRITICAL"] }, dueDate: { type: ["string", "null"], format: "date-time" }, assigneeId: { type: ["string", "null"], format: "uuid" }, assignee: { oneOf: [{ $ref: "#/components/schemas/UserSummary" }, { type: "null" }] }, createdById: uuid, createdBy: { $ref: "#/components/schemas/UserSummary" }, estimatedHours: nullableString, actualHours: nullableString, archived: { type: "boolean" }, commentCount: { type: "integer" }, attachmentCount: { type: "integer" }, createdAt: dateTime, updatedAt: dateTime }),
  ...
  Comment: entity({ id: uuid, taskId: uuid, organizationId: uuid, authorId: uuid, author: { type: "object", properties: { id: uuid, email: { type: "string", format: "email" }, firstName: { type: "string" }, lastName: { type: "string" }, avatar: nullableString, isDeleted: { type: "boolean" } }, required: ["id", "email", "firstName", "lastName", "avatar", "isDeleted"] }, content: { type: "string", maxLength: 5000 }, edited: { type: "boolean" }, createdAt: dateTime, updatedAt: dateTime }),
  Attachment: entity({ id: uuid, organizationId: uuid, taskId: uuid, uploadedBy: uuid, originalName: { type: "string" }, storedName: { type: "string" }, mimeType: { type: "string" }, fileSize: { type: "integer", maximum: 26214400 }, provider: { type: "string" }, status: { type: "string", enum: ["PENDING", "READY"] }, uploadExpiresAt: { type: ["string", "null"], format: "date-time" }, finalizedAt: { type: ["string", "null"], format: "date-time" }, extractionStatus: { type: "string", enum: ["NOT_REQUESTED", "PENDING", "PROCESSING", "COMPLETED", "FAILED", "UNSUPPORTED"] }, extractionErrorCode: nullableString, extractionRequestedAt: { type: ["string", "null"], format: "date-time" }, extractionStartedAt: { type: ["string", "null"], format: "date-time" }, extractionCompletedAt: { type: ["string", "null"], format: "date-time" }, extractorName: nullableString, extractorVersion: nullableString, extractedCharacterCount: { type: ["integer", "null"] }, extractionTruncated: { type: "boolean" }, createdAt: dateTime }),
  AttachmentExtraction: entity({ attachmentId: uuid, status: { type: "string", enum: ["NOT_REQUESTED", "PENDING", "PROCESSING", "COMPLETED", "FAILED", "UNSUPPORTED"] }, supported: { type: "boolean" }, extractorName: nullableString, extractorVersion: nullableString, characterCount: { type: ["integer", "null"] }, truncated: { type: "boolean" }, errorCode: nullableString, requestedAt: { type: ["string", "null"], format: "date-time" }, startedAt: { type: ["string", "null"], format: "date-time" }, completedAt: { type: ["string", "null"], format: "date-time" } }),
  AttachmentExtractedText: entity({ attachmentId: uuid, status: { type: "string", enum: ["NOT_REQUESTED", "PENDING", "PROCESSING", "COMPLETED", "FAILED", "UNSUPPORTED"] }, supported: { type: "boolean" }, extractorName: nullableString, extractorVersion: nullableString, characterCount: { type: ["integer", "null"] }, truncated: { type: "boolean" }, errorCode: nullableString, requestedAt: { type: ["string", "null"], format: "date-time" }, startedAt: { type: ["string", "null"], format: "date-time" }, completedAt: { type: ["string", "null"], format: "date-time" }, text: nullableString }),
  ...
  AiHealth: entity({ enabled: { type: "boolean" }, configured: { type: "boolean" }, available: { type: "boolean" }, status: { type: "string", enum: ["disabled", "healthy", "degraded", "unavailable"] }, provider: { type: "string" }, model: { type: "string" }, checkedAt: dateTime, latencyCategory: { type: "string", enum: ["fast", "normal", "slow", "timeout"] }, degradedReasonCode: { type: "string" }, reason: { type: "string" }, rateLimit: entity({ store: { type: "string", enum: ["memory", "redis"] }, distributed: { type: "boolean" }, windowMs: { type: "integer" }, userLimit: { type: "integer" }, organizationLimit: { type: "integer" }, available: { type: "boolean" }, detail: { type: "string" } }), persistence: entity({ promptsStored: { type: "boolean", const: false }, responsesStored: { type: "boolean", const: false }, conversationHistoryStored: { type: "boolean", const: false } }), mode: { type: "string", enum: ["read_only"] } }),
};
...
const openApiDefinition = {
  ...
  paths: {
    ...
    "/ai/copilot/query": { post: { tags: ["AI"], summary: "Ask the read-only Business Copilot", security, requestBody: body("#/components/schemas/AiCopilotRequest"), responses: { 200: ok({ $ref: "#/components/schemas/AiCopilotResponse" }), 400: errors[400], 401: errors[401], 403: errors[403], 404: errors[404], 429: { description: "AI rate limit exceeded", ...content({ $ref: "#/components/schemas/ErrorResponse" }) }, 503: { description: "AI provider unavailable", ...content({ $ref: "#/components/schemas/ErrorResponse" }) }, 504: { description: "AI provider timeout", ...content({ $ref: "#/components/schemas/ErrorResponse" }) } } } },    "/health": { get: { tags: ["Health"], summary: "Health and safe dependency readiness", responses: { 200: { description: "API is live", ...content(entity({ success: { type: "boolean", const: true }, status: { type: "string", enum: ["ok", "degraded"] }, dependencies: entity({ redis: entity({ enabled: { type: "boolean" }, required: { type: "boolean" }, available: { type: "boolean" }, status: { type: "string" }, failureCategory: { type: ["string", "null"] } }) }) })) }, 503: { description: "Required dependency unavailable", ...content(entity({ success: { type: "boolean", const: true }, status: { type: "string", const: "degraded" } })) } } } },
    ...
    "/attachments/{id}/extraction": { post: { tags: ["Attachments"], summary: "Request attachment text extraction", security, parameters: [id("id")], responses: { 202: wrapped("extraction", "#/components/schemas/AttachmentExtraction"), ...errors } }, get: securedGet("Attachments", "Get attachment extraction status", wrapped("extraction", "#/components/schemas/AttachmentExtraction"), [id("id")]) },
    "/attachments/{id}/extraction/retry": { post: { tags: ["Attachments"], summary: "Retry attachment text extraction", security, parameters: [id("id")], responses: { 202: wrapped("extraction", "#/components/schemas/AttachmentExtraction"), ...errors } } },
    "/attachments/{id}/extraction/text": { get: securedGet("Attachments", "Get extracted attachment text", wrapped("extraction", "#/components/schemas/AttachmentExtractedText"), [id("id")]) },
